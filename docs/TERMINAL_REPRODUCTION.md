...
> **Historical record of one run, at commit `c4d250d`. Superseded.**
> The counts below were correct then and are not correct now: the
> suites and the gate set have both grown since. They are kept because
> the defects each run found are worth preserving, not because the
> numbers are current.
>
> For the current state see `verification/REPRODUCTION_RECORD.md`,
> which is generated by `scripts/reproduce_all.py` and cannot go stale
> the way this file did.
