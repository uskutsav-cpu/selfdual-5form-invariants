...
Generated 2026-08-01T22:26:14+00:00 by `scripts/emit_degree10_space_status.py`.
...
## B10 rational reconstruction --- practical, not done

The route exists and is worth naming rather than dismissing. B10's
coefficients are modular images at four primes, giving a CRT modulus of
about 1e18, so a rational lift is unique when numerator and denominator
are each below roughly 1e9. Whether the entries are that small is not
known; the published-candidate solves would have to be rerun at more
primes and the lifts revalidated on unused ones.

It was not attempted here because the machine is committed to the
certificate matrix and no claim in this manuscript depends on it. The
finite-field qualification is retained rather than quietly dropped.
